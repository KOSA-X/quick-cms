...
if( isset( $config['current_page_id'] ) && is_numeric( $config['current_page_id'] ) && isset( $oPage->aPages[$config['current_page_id']] ) ){
  ...
  $config['title'] = trim( !empty( $aData['sTitle'] ) ? str_replace( '&quot;', '"', $aData['sTitle'] ).' - '.$config['title'] : ( ( !isset( $config['display_homepage_name_title'] ) && $config['current_page_id'] == $config['start_page'] ) ? $config['title'] : strip_tags( $aData['sName'] ).' - '.$config['title'] ) );

  if( !empty( $aData['sDescriptionMeta'] ) )
    $config['description'] = str_replace( '&quot;', '\'', $aData['sDescriptionMeta'] );
